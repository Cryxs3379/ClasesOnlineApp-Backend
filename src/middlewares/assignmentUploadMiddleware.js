...
const { getAssignmentUploadPath, ensureDirectoryExists } = require('../utils/fileUtils');
...
const uploadDir = getAssignmentUploadPath();
  ...
  upload.single('file')(req, res, (error) => {
    if (error instanceof multer.MulterError) {
      if (error.code === 'LIMIT_FILE_SIZE') {
        return next(new AppError('El archivo no puede superar los 10 MB', 400));
      }
